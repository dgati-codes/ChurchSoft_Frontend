...
const StatCard = ({ title, value, subtitle, icon, color }) => (
  <div className="bg-white shadow rounded-lg p-4 flex flex-col gap-1 border">
    ...
    <div className="min-h-screen -ml-64 bg-gray-100 ">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowAddAttendanceRecord(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg">
            + Add Record
          </button>
          <button className="bg-blue-600 text-white px-4 py-2 rounded-lg">
            + Add Notes
          </button>
          <button className="border px-4 py-2 rounded-lg">Export PDF</button>
          <button className="border px-4 py-2 rounded-lg">Print</button>
        </div>
      </div>

      {/* Filters */}
      <div className="bg-white p-4 m-4 rounded-lg shadow border">
      ...
      <div className="grid grid-cols-1 m-4 md:grid-cols-4 gap-4">
        <StatCard title="Total Attendance" value="1550" subtitle="Across 1 service" />
        <StatCard title="Average Attendance" value="189" subtitle="Per Service" />
        <StatCard title="Active Locations" value="16" subtitle="16 regions" />
        <StatCard title="Service Records" value="32" subtitle="Total entries" />
      </div>

      {/* Demographics Breakdown */}
      <div className="bg-white mb-4 rounded-lg shadow border p-4">
